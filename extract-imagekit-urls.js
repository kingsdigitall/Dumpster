...
function findImagekitKeys(obj, topLevelOnly = false) {
  ...
  if (Array.isArray(obj)) {
    obj.forEach((item, index) => {
      if (typeof item === 'object' && item !== null) {
        ...
        // Store with index to preserve array structure
        if (Object.keys(itemKeys).length > 0) {
          result[index.toString()] = itemKeys;
        }
      }
    });
    return result;
  ...
  for (const [key, value] of Object.entries(obj)) {
    ...
    } else if (!topLevelOnly && Array.isArray(value)) {
      // Handle arrays within objects
      const arrayResult = findImagekitKeys(value, false);
...
function processJsonFile(filePath) {
  try {
    ...
    // For Blogs.json, handle array structure
    if (fileName === 'Blogs') {
      const blogsFilePath = path.join(__dirname, 'app', 'components', 'Blogs.json');
      if (filePath === blogsFilePath) {
        const result = {};
        if (Array.isArray(jsonData)) {
          jsonData.forEach((blog, index) => {
            const blogUrls = extractImagekitUrlsFromItem(blog);
            if (Object.keys(blogUrls).length > 0) {
              result[index.toString()] = blogUrls;
            }
          });
        }
        return result;
...
async function extractAllImagekitUrls() {
  ...
  // Also process Blogs.json in app/components
  const blogsFilePath = path.join(__dirname, 'app', 'components', 'Blogs.json');
  if (fs.existsSync(blogsFilePath)) {
    console.log(`Processing Blogs.json...`);
    const blogsResult = processJsonFile(blogsFilePath);
    if (Object.keys(blogsResult).length > 0) {
      output['Blogs'] = blogsResult;
    }
  }
  ...
  console.log(`\nTotal files processed: ${files.length + 1}`); // +1 for Blogs.json
